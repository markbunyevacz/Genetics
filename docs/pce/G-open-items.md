...
`LIVE_CDS` a repóban **false**. Az (a) kill-switch **megvan** (`src/pce_cds/policy.py`, `IIA_SAFE_BLOCK=true`). Az (a) az *első live kiadás* termékdöntése; a pecsét a flaget billenti, nem a csövet írja újra. A blokk a fenti öt pár **mechanizmus-családját** fedi a pinelt CPIC rec_view / opioid guideline szerint (tramadol a kodeinnel, tegafur a fluoropirimidinnel, tioguanin a tiopurinnal, oxkarbazepin/fenitoin a HLA-B\*15:02 ággal), plusz magyar INN-variáns (`klopidogrel`, `karbamazepin`, …). Ez **nem** új Rule 11 pecsét.

A WHO 4. szintű `L01BC*` / `L01BB*` prefix **nem** mechanizmus. Pinelt ellenpélda (S076, 2026-08-16): gemcitabin L01BC05 és citarabin L01BC01 a „Pyrimidine analogues” alatt; fludarabin L01BB05 és kladribin L01BB04 a „Purine analogues” alatt. Egyiknek sincs DPYD- vagy TPMT-párja a CPIC `pair_drugnames`-ben. A catch-all hamis pozitív lett volna — a klinikus leszokna a blokkról.
