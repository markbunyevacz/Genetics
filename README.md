...
Work only on **`main`**. `LIVE_CDS` and the F1+ matcher are compile-time **false**. The F2 CDS pipe (`pce_cds`) is in the box; the prescriber path stays locked.
...
PYTHONPATH=src python3 -m pce_cds --port 8092
